...
def process_amass(smooth_n=4):
    ...
    body_model = art.ParametricModel(paths.male_smpl_file)
...
def generate_amass():
    def filter_less_seqLeng(x):
        return x.shape[0] == seq_len
    pose = torch.load(os.path.join(paths.amass_dir, 'pose.pt'))
    shape = torch.load( os.path.join(paths.amass_dir, 'shape.pt'))
    tran = torch.load(os.path.join(paths.amass_dir, 'tran.pt'))
    joint = torch.load(os.path.join(paths.amass_dir, 'joint.pt'))
    vrot = torch.load(os.path.join(paths.amass_dir, 'vrot.pt'))
    vacc = torch.load(os.path.join(paths.amass_dir, 'vacc.pt'))

    length = len(pose)
    seq_len = 200
    out_data = {"pose":[], "tran":[], "joint":[], "ori":[], "acc":[]}
    for i in range(length):
        pose_arr = list(filter(filter_less_seqLeng, torch.split(pose[i], seq_len)))
        if pose_arr:out_data['pose'].extend(pose_arr)

        tran_arr = list(filter(filter_less_seqLeng, torch.split(tran[i], seq_len)))
        if tran_arr:out_data['tran'].extend(tran_arr)

        joint_arr = list(filter(filter_less_seqLeng, torch.split(joint[i], seq_len)))
        if tran_arr:out_data['joint'].extend(joint_arr)

        ori_arr = list(filter(filter_less_seqLeng, torch.split(vrot[i], seq_len)))
        if tran_arr:out_data['ori'].extend(ori_arr)

        acc_arr = list(filter(filter_less_seqLeng, torch.split(vacc[i], seq_len)))
        if tran_arr:out_data['acc'].extend(acc_arr)

    torch.save(out_data, os.path.join(paths.amass_dir, 'transPose_dataset.pt'))
